...
w2 = 3e-6 #3e-6
zfoc = 26e-6 #26e-6 70 is really good 
E20 = 0.1
...
nz = len(z)
#mid_index = nz // 2  # Insert the lens at this index
mid_index = 25 #this is not currently the midpoint but i wanted to move the lens around freely so changed the value
...
plt.axhline(y = 2.5e-5, label='Lens Position')
plt.axhline(y = zfoc, color='red', label='Focus')
plt.xlabel('x (m)')
plt.ylabel('z (m)')
plt.title('Propagation with a Lens at z = {:.1e} m'.format(z[mid_index]))
