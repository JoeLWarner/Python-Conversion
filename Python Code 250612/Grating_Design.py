...
#  target beam section
xv = np.linspace(-15e-6, 15e-6, 12000)  #  create grid in the grating plane
...
# def sech(x):
#     return 1/np.cosh(x)
# soliton_width = 5e-6  # width parameter
...
# Instead of taking sqrt of a negative real, allow complex kz:
...
plt.figure(figsize=(8,6))
plt.pcolormesh(x, z, np.abs(Etot)**2, shading='auto', cmap='jet')
plt.xlabel('x (m)')
plt.ylabel('z (m)')
...
# =============================================================================
# plt.figure(figsize=(8, 6))
# phase_data = np.angle(Etot)  # Extract phase information (-π to π)
# plt.pcolormesh(x, z, phase_data, cmap='twilight', shading='auto')
# plt.xlabel('x (m)')
# plt.ylabel('z (m)')
...
# 
# z_focus_target = zfoc 
# idx_focus = np.argmin(np.abs(z - z_focus_target))
# 
# idx_initial = 2  
# 
# selected_indices = [idx_initial, 50]
# selected_z_values = [z[i] for i in selected_indices]
# 
# fig, axs = plt.subplots(1, 2, figsize=(18, 6))
# 
# for j, idx in enumerate(selected_indices):
#     intensity_profile = np.abs(Etot[idx, :])**2
...
#     axs[j].set_xlim([-25, 25])
#     axs[j].grid(True)
#     axs[j].set_title('z = {:.1e} m\nFWHM = {:.1e} m'.format(z[idx], fwhm_val), fontsize=12)
#     #axs[j].legend()
# 
# plt.tight_layout()
# plt.show()
