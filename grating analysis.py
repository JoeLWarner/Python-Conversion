...
for period in grating_period_range:
    ...
    phi_list.append(phi)
    theta = phi/2
    afg, _, _, _ = scatteringrate2D_f(period, theta, pol, k0, neff, nsil, w0, dng)
...
for angle in diff_angles:
    ...
    for width in w_values:
        al1, _, _, _ = scatteringrate2D_f(Lambda, theta, pol, k0, neff, nsil, width, 3e-3)
...
neff_values = [1.459147, 1.455880, 1.453487, 1.4449355552629308]
waist_values = [1.832930e-06, 2.145548e-06, 2.456798e-06, 3.983035e-06]
wave_range = [650e-9, 780e-9, 910e-9, 1550e-9]
op_value = [1, 0.7, 0.3, 0.9]
colours = ['black', 'red', 'blue', 'lime']
plt.figure(figsize=(8, 6))
...
for i, l in enumerate(wave_range):
    ...
    for angle in diff_angles:
        ...
        alpharange_list = []
        for phi in diff_range:
            alpharange, _, _, _ = scatteringrate2D_f(Lambda, phi, 1, k0, neff, nsil, waist_values[i], 1e-3)
